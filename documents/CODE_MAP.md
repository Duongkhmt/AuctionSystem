...
- **Bảo mật 2 Lớp (2-Layer Security):** `✅ Confirmed` Spring Security Stateless JWT Authentication với RBAC 3-tier routes ở `auction-service` ([SecurityConfig.java](file:///home/duong/Projects/Backend/auction-service/src/main/java/com/duong/auction/system/config/SecurityConfig.java)) và 2-Layer Guard ở `payment-service` (Lớp 1: Feign `X-Internal-Service-Key`, Lớp 2: `/v1/wallets/me` JWT `@AuthenticationPrincipal Long userId` 0ms SQL lookup).
