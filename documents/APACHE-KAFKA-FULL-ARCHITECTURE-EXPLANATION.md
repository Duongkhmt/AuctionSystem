...
# 🎯 5. HƯỚNG DẪN TRẢ LỜI PHỎNG VẤN VỀ KAFKA TRONG DỰ ÁN

Khi nhà tuyển dụng hỏi: *"Bạn đã áp dụng Kafka vào dự án như thế nào?"*

👉 **Bạn trả lời chuẩn chuyên gia:**
> *"Em dùng Kafka để xử lý bất đồng bộ sự kiện `AUCTION_ENDED`. Luồng chính của Robot Scheduler chỉ làm nhiệm vụ chốt trạng thái phiên thầu và bắn Event lên Kafka trong dưới 2ms. Phía ngầm có Kafka Consumer nhặt sự kiện để tạo Đơn hàng 48h và gửi thông báo. Để đảm bảo an toàn hệ thống, em áp dụng cơ chế Non-Blocking Retry Topic 3 tầng với Dead Letter Topic (DLT), chống Dual-Write bằng Spring `afterCommit`, tách Helper Bean tránh lỗi Spring AOP Self-Invocation, và thiết kế Consumer Idempotency chuẩn Check-Then-Mark dùng Redis 24h."*
